...
const onCameraBtn = document.getElementById('onCameraBtn');
const offCameraBtn = document.getElementById('offCameraBtn');
...
function calculatePerformanceScore() {
    // Send activity scores to backend and redirect to performance page
    fetch('/performance', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        // Store the performance data in sessionStorage for the performance page
    ...
    .catch(error => {
        ...
        alert('Error calculating performance. Please try again.');
    });
}
...
function startGlobalRecognition() {
    // Global on camera function for athlete fitness
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
                ...
                if (video) {
                    ...
                    intervalId = setInterval(() => {
                        if (video.videoWidth > 0) {
                            ...
                            .then(response => response.json())
                            .then(data => {
                                if (activitySpan) activitySpan.textContent = `activity - ${data.activity}`;
                                if (scoreSpan) scoreSpan.textContent = data.score;
                                ...
                                if (data.activity in activityScores) {
                                    ...
                                    updateAverageScore();
                                    // Update individual activity score display
                                    const activityItem = document.querySelector(`[data-activity="${data.activity}"]`);
                                    if (activityItem) {
                                        const scoreElement = activityItem.parentElement.querySelector('.activity-score');
                                        if (scoreElement) {
                                            scoreElement.textContent = `Score: ${data.score}`;
    ...
    alert('Global camera recognition started.');
}

function stopGlobalRecognition() {
    // Global off camera function
    if (stream) {
        stream.getTracks().forEach(track => track.stop());
        stream = null;
    }
    ...
    if (activitySpan) activitySpan.textContent = 'Waiting...';
    if (scoreSpan) scoreSpan.textContent = '0.00';
    alert('Global camera recognition stopped.');
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editProfileBtn) {
        editProfileBtn.addEventListener('click', function() {
            ...
            if (uploadPhotoForm) uploadPhotoForm.style.display = 'none';
        });
    }
    ...
    if (profileForm) {
        profileForm.addEventListener('submit', function(e) {
            e.preventDefault();
            // For now, just hide the form (in a real app, you'd send data to server)
